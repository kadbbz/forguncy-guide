...
::: info
项目确定下来转为正式开发后，再遵循 [模型设计](/standard/dev/database/domain) 进行优化，利用「内建表转为外联表」，将其转到外联数据库中继续开发。
:::
...
【<font color="#1677FF">推荐</font>】进入测试阶段后，需要建立测试环境和持续发布机制，确保测试环境及时更新，以提升测试工作效率。更多请参考：[配置管理](/standard/dev/configuration/application#环境)。
...
【<font color="#1677FF">推荐</font>】升级过程中会涉及到对协同仓库的分支进行覆盖与删除，需确保主分支与生产环境保持一致。更多请参考：[版本控制与协作](/standard/dev/cooperation)。

## 线上运维

【<font color="#1677FF">推荐</font>】为了提升可维护性，在构建生产环境时，首先需要配置文件服务器和数据库服务器。不推荐在生产环境中使用默认的用户信息数据库和各种存储路径。
![生产环境配置](/standard/images/arch-life-prd.png "生产环境配置")
...
【<font color="#1677FF">推荐</font>】为不同的环境启用不同的日志策略，详情请参考：[服务配置-日志策略](/standard/operation/service)。
